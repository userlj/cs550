...
int main(int argc, char** argv) {
	...
	if (argc <= 2) {
		...
		printf("MAX_MEMORY stands for the log2(max size of memory)"
                "you want to allocate.\n");
		printf("ITERATIONS stands for the iteration times of the 4 "
                "operations you want to test.\n");
		printf("\n");
		printf("Note that it might exceed MAX memory when using "
                "calloc(block size 2)!\n");
		printf("So choose your MAX memory between 1 and 29 if you are "
                "using 32bit machine!\n");
		exit(1);
	}

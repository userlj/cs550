...
void *malloc(size_t size)
{
	...
	else
	{
		...
		int diff = 8 - (((unsigned long)new_addr)&7);   
		// make the address to multiple of 8
		sbrk(diff);
		new_addr = sbrk(size);
		// get the index for alloc_list
		int alloc_list_index = (((unsigned long)new_addr)%ALLOC_LIST_SIZE);  
...
void *realloc(void *ptr, size_t size)
{
	...
	else
	{
		...
		{
			// get the index of alloc_list for ptr
			unsigned long ul_ptr = (unsigned long) ptr;
	        	int alloc_list_index = ul_ptr%ALLOC_LIST_SIZE;
        		struct memBlk *tmp = alloc_list[alloc_list_index];  
        		while(tmp->addr!=ptr)
			...
			// if the size of old block is large enough
			// use the old block
			if(size<=(tmp->size)) return tmp->addr;   
			// if the size of old block is not enough
			else
			{
